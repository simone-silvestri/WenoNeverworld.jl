...
ax = Axis(fig[1, 1], xlabel="longitude [∘]", xlabelsize = 30, xticklabelsize = 30, ylabel="latitude [∘]", ylabelsize = 30,title="Deformation radius, 1/4∘ " ,  yticklabelsize = 30, titlesize=50)
hm = heatmap!(ax, collect(lon), collect(lat), log10.(ross_2d .+ eps(1000.0)), colorrange = (3, 5), aspect_ratio = 0.8,colormap = :plasma)

#contour lines
contour_levels = 4
contour_lines = contour!(ax, collect(lon), collect(lat), log10.(ross_2d .+ eps(1000.0)), levels=range(3.2, stop=4.8, length=contour_levels+2)[2:end-1], linewidth=3, linecolor=:black, labels=true, labelsize = 30, labelfont = :bold, labelcolor = :black)


cbar1 = Colorbar(fig[1,2], hm, width = 30, ticksize = 30)
display(fig)
save("plotting/weno_half_ross.png", fig)
##

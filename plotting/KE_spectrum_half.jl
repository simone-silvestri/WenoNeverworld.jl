...
prefixes = ["weno_half_original_ch", "weno_half_larger_diffusivity_ch", "weno_half_larger_viscosity_ch", "weno_half_variable_diff_ch"]
dirs = ["/storage4/WenoNeverworldData/half_degree/", "/storage4/WenoNeverworldData/half_degree/", "/storage4/WenoNeverworldData/half_degree/", "/storage4/WenoNeverworldData/half_degree/"]
resolutions = ["1/2 original", "1/2 larger diff", "1/2 larger visc + diff", "1/2 variable diff"]
colors = [:blue, :red, :green, :purple]


xrange = [11 : 60 * i - 10  for i in [2, 2, 2, 2]]
y50S   = [42  * i : 48  * i for i in [1, 1, 1, 1]]
y15N   = [160 * i : 170 * i for i in [1, 1, 1, 1]]
y50N   = [210 * i : 220 * i for i in [1, 1, 1, 1]]
...
l5 = lines!(ax3, F16_50S[20:end-8], F16_50S[20:end-8].^(-3) ./ 10^(17.5), linewidth = 2.5, color = :black)#, linestyle = :dashdot)
...
leg = Legend(fig[1, 4],
    [l1, l2, l3, l4, l5],
    ["1/2 original", "1/2 larger diff", "1/2 larger visc + diff", "1/2 variable diff", "-3 slope"], labelsize=25)
...
using CairoMakie
CairoMakie.activate!()
CairoMakie.save("ke_spectrum_half4.png", fig, px_per_unit = 3)

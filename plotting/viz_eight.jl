...
@info "Loading data..."
path = pwd()
hfile = jldopen(path * "/data/weno_eight.jld2", "r")
keys(hfile)
...
for (i,lon_index) in enumerate(longitudes) 
    ...
    display(fig)
    save("weno_eight_b_" * string(i) * ".png", fig)
end
...
cbar1 = Colorbar(fig[1,2], hm, width = 30, ticksize = 30)
display(fig)
save("weno_eight_tke_slice.png", fig)
##

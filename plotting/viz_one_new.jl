...
@info "Loading data..."
path = pwd()
hfile = jldopen("/storage2/WenoNeverworldData/weno_one_checkpoint_iteration9993525.jld2", "r")
keys(hfile)
...
for (i,lon_index) in enumerate(longitudes) 
    ...
    display(fig)
    save("plotting/weno_one_new_b_" * string(i) * ".png", fig)
end
...
display(fig)
save("plotting/weno_one_new_tke.png", fig)
##
...
cbar1 = Colorbar(fig[1,2], hm, width = 30, ticksize = 30)
display(fig)
save("plotting/weno_one_new_tke_slice.png", fig)
##

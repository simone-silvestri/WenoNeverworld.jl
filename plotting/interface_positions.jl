...
function load_data_and_contours(path::String)
    ...
    lon_index = argmin(abs.(lon .- oceangrid.underlying_grid.λᵃᶜᵃ)) # Find the closest longitude index
    return b, lat, z, contours, lon_index
    
end

# Define the file paths for each degree resolution
path_1 = "/storage2/WenoNeverworldData/weno_sixteenth_checkpoint_iteration972420.jld2" # #plotting/weno_fourth.jld2"#fourth data path         #resolution 1 data
path_2 =  "/storage2/WenoNeverworldData/weno_one_checkpoint_iteration9993525.jld2" #weno_eighth_checkpoint_iteration985500.jld2" #half data path  #resolution 2 data
...
cl = contour!(ax,lat_1,  z_1, b_1[lon_index_1,:, :]; levels =  contours_1, colormap =  :blues)
...
c2 = contour!(ax,lat_2,  z_2, b_2[lon_index_2,:, :]; levels =  contours_2, colormap =  :reds)

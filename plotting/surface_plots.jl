...
cb  = Colorbar(fig[0, 2], hm1, vertical = false, label = L"\text{Surface Vertical Vorticity [s}^{-1} \cdot 10^{-5}\text{]}",
               ticks = ([-3e-5, 0, 3e-5], [L"-3", L"0", L"3"]))
hidedecorations!(ax)
hidespines!(ax)
display(fig)
using CairoMakie
CairoMakie.activate!()
CairoMakie.save("vort_thirtytwo.png", fig, px_per_unit = 5)
...
cb  = Colorbar(fig[0, 3], hm1, vertical = false, label = L"\text{Surface Temperature [}^\circ\text{C}^{-1}\text{]}",
               ticks = ([0, 10, 20, 30], [L"0", L"10", L"20", L"30"]))
hidedecorations!(ax)
hidespines!(ax)
display(fig)
using CairoMakie
CairoMakie.activate!()
CairoMakie.save("surf_temp_thirtytwo.png", fig, px_per_unit = 5)
...
ax = Axis(fig[1, 1], xlabel="longitude [∘]", xlabelsize = 30, xticklabelsize = 30, ylabel="latitude [∘]", ylabelsize = 30,title="Deformation radius, 1/2∘ " ,  yticklabelsize = 30, titlesize=50)
...
contour_lines = contour!(ax, collect(lon), collect(lat), log10.(ross_2d .+ eps(1000.0)), levels=contour_labels, linewidth=3,  labelformat="%.1f", linecolor=:black, labels=true, labelsize = 30, labelfont = :bold, labelcolor = :black)

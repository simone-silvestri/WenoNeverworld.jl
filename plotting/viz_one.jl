...
@info "Loading data..."
path = pwd()
hfile = jldopen(path * "/simulations_one/Neverworld_one_checkpoint_iteration658110.jld2", "r")
keys(hfile)
#taking data from earlier GPU initialization run, not interpolating from 1/2
...
for (i,lon_index) in enumerate(longitudes) 
    ...
    display(fig)
    save("plotting/weno_one_b_" * string(i) * ".png", fig)
end
...
display(fig)
save("plotting/weno_one_tke.png", fig)
##
...
cbar1 = Colorbar(fig[1,2], hm, width = 30, ticksize = 30)
display(fig)
save("plotting/weno_one_tke_slice.png", fig)
##

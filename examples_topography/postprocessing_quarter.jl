...
center = limit_timeseries!(center, center[:b].times[end-40:end])

weno = all_fieldtimeseries("files_four/neverworld_quarter_snapshots.jld2")
weno = limit_timeseries!(weno, weno[:b].times[end-40:end])

wenon = all_fieldtimeseries("files_four_new_bathy/neverworld_quarter_snapshots.jld2")
wenon = limit_timeseries!(wenon, wenon[:b].times[end-40:end])
...
WenoNeverworld.Diagnostics.add_kinetic_energy_and_vorticity_timeseries!(wenon)
...
Êwenon  = average_spectra(wenon[:E],  Colon(), 78:82; k = 65)

Ω̂center = average_spectra(center[:ζ], Colon(), 78:82; k = 65)
Ω̂weno   = average_spectra(weno[:ζ],   Colon(), 78:82; k = 65)
Ω̂wenon  = average_spectra(wenon[:ζ],  Colon(), 78:82; k = 65)
...
lines!(ax, fxE[2:end], Êwenon.spec[2:end],  color = :green, label = "WENO momentum scheme")
...
lines!(ax, fxζ[2:end], Ω̂wenon.spec[2:end],  color = :green, label = "WENO momentum scheme")
...
# center_avg = all_fieldtimeseries("files_four_centered/neverworld_quarter_centered_averages.jld2"; variables)
# center_avg = limit_timeseries!(center_avg, center_avg[:b].times[end-20:end])

# weno_avg = all_fieldtimeseries("files_four/neverworld_quarter_averages.jld2"; variables)

...
halo = 7
grid = jldopen("/storage4/WenoNeverworldData/sixteenth_degree/weno_sixteen_checkpoint_iteration2417760.jld2", "r") do h
    h["grid"]
end
...
diffs = Dict(
   ...
   #"1/16−1/" => (b16_4 .- b_zonal["quarter_degree"],  lat_4),
...
for (tag, (diff, latsub)) in diffs
  ...
  hm = heatmap!(ax, latsub, z, diff, colormap=:balance)
...
out = "figures/zonal_avg_diff_2.png"
  save(out, fig)
  @info "Saved $out"
end

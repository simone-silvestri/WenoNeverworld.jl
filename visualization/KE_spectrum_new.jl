...
# Define a function to find the closest latitude
function closest_lat_index(lats, target_lat)
    return argmin(abs.(lats .- target_lat))
end
index_60N = closest_lat_index(lats, 60)
...
ax4 = Axis(fig[1, 4], xscale = log10, yscale = log10, yticklabelsize = 25, xticklabelsize = 25,
		      # title  = L"37^\text{o}\text{N}")
		      xlabelsize = 20, xlabel = "k [1/m]")
...
using CairoMakie
CairoMakie.activate!()
CairoMakie.save("figures/ke_spectrum3.png", fig, px_per_unit = 3)

...
@info "Loading data..."
path = pwd()
hfile = jldopen("/storage4/WenoNeverworldData/eighth_degree_new/weno_eighth_checkpoint_iteration3438720.jld2", "r")
keys(hfile)
...
hm = heatmap!(ax, lon, lat, log10.(weighted_tke_slice .+ eps(1000.0)), colorrange = (-4, 1), colormap = :plasma, aspect = 0.8)
...
using CairoMakie
CairoMakie.activate!()
CairoMakie.save("figures/KE_eighth.png", fig, px_per_unit = 5)
##

...
prefixes = ["weno_half_ch", "weno_quarter__ch", "weno_eighth_ch", "weno_sixteenth_ch", "weno_thirtytwo_comp"]
dirs = ["/storage4/WenoNeverworldData/half_degree_new/", "/storage4/WenoNeverworldData/quarter_degree_new/", "/storage4/WenoNeverworldData/eighth_degree_new/", "/storage3/WenoNeverworldData/", "/storage4/WenoNeverworldData/"]
resolutions = ["1/2", "1/4", "1/8", "1/16", "1/32"]
colors = [:red3, :darkorange, :green, :purple, :navy]


xrange = [11 : 60 * i - 10  for i in [2, 4, 8, 16, 32]]
y50S   = [42  * i : 48  * i for i in [1, 2, 4, 8, 16]]  #somehow correct?
y15N   = [160 * i : 170 * i for i in [1, 2, 4, 8, 16]]  # 12 N
y50N   = [210 * i : 220 * i for i in [1, 2, 4, 8, 16]]  # 37 N
...
ax1 = Axis(fig[1, 1], xscale = log10, yscale = log10, yticklabelsize = 25, xticklabelsize = 25,
		      ...
		      ylabelsize = 20, xlabelsize = 20, xlabel = "k [1/m]", 
...
lines!(ax1, F8_50N[20:end-8], F8_50N[20:end-8].^(-3) ./ 10^(17.5), linewidth = 2.5, color = :black)#, linestyle = :bold)
...
lines!(ax2, F8_15N[20:end-8], F8_15N[20:end-8].^(-3) ./ 10^(18.5), linewidth = 2.5, color = :black)#, linestyle = :bold)
...
leg = Legend(fig[1, 4],
    [l1, l2, l3, l6, l7],
    ["1/2°", "1/4°", "1/8°", "-3 slope", ""], labelsize = 25)
...
using CairoMakie
CairoMakie.activate!()
CairoMakie.save("figures/ke_spectrum2.png", fig, px_per_unit = 3)

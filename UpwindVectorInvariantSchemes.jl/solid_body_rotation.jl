...
using Oceananigans.Advection: VelocityStencil, VorticityStencil, EnergyConservingScheme, EnstrophyConservingScheme
...
function run_solid_body_rotation(; architecture = CPU(),
    ...
    wave_propagation_time_scale = min_Δx(grid) / gravity_wave_speed
...
function plot_zonal_average_solid_body_rotation(filepath)
    ...
    GLMakie.record(fig, "zonally_averaged_solid_body_rotation_Nx$Nx.mp4", iterations, framerate=30) do i
...
filepath = run_solid_body_rotation(super_rotations=0.5, advection_scheme=VectorInvariant(), prefix = "2ndorder")
plot_zonal_average_solid_body_rotation(filepath)
filepath = run_solid_body_rotation(super_rotations=0.5, advection_scheme=WENO(vector_invariant = VelocityStencil()), prefix = "weno")
plot_zonal_average_solid_body_rotation(filepath)
filepath = run_solid_body_rotation(super_rotations=0.5, advection_scheme=GlobalVectorInvariant(upwind_scheme = WENO()), prefix = "global")
plot_zonal_average_solid_body_rotation(filepath)
